...
def wrap_model_with_fsdp(model, args):
    ...
    torch.cuda.empty_cache()
    logger.info(f"After model wrap")
    return model
...
def main():
    ...
    if args.total_batch_size is not None:
        ...
        if args.total_batch_size % (world_size * args.per_device_train_batch_size) != 0:
            raise ValueError(f"`--total_batch_size` ({args.total_batch_size}) is not divisible by "
                             f"num_processes * per_device_train_batch_size ({world_size} * {args.per_device_train_batch_size})")
        logger.info(f"world_size: {world_size}")
        args.gradient_accumulation_steps = args.total_batch_size // (args.per_device_train_batch_size * world_size)
        logger.info(f"Setting gradient accumulation steps to {args.gradient_accumulation_steps}.")
    else:
        args.total_batch_size = args.gradient_accumulation_steps * args.per_device_train_batch_size * world_size
    ...
    # Distributed stuff
    dist_sampler = functools.partial(
    ...
    train_dataloader = DataLoader(
        ...
        sampler=dist_sampler(dataset=train_dataset, shuffle=True),
    )
    eval_dataloader = DataLoader(
        eval_dataset,
        collate_fn=data_collator,
        batch_size=args.per_device_eval_batch_size,
        sampler=dist_sampler(dataset=eval_dataset, shuffle=False),
    )
    ...
    if "test" in raw_datasets:
        test_dataloader = DataLoader(
            ...
            collate_fn=data_collator,
            batch_size=args.per_device_eval_batch_size,
            sampler=dist_sampler(dataset=test_dataset, shuffle=False),
        )
    ...
    experiment_config["world_size"] = world_size
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            if (update_step + 1) % args.eval_every_steps == 0 or (update_step == 1 and not args.skip_initial_eval):
                ...
                wandb.log(result, step=update_step)
    ...
    if best_results_dict is not None:
        wandb.log(best_results_dict, step=update_step)
    else:
